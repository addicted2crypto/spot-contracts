...
contract SPOTbot is ERC721URIStorage, IERC721Receiver, ReentrancyGuard, Ownable {
  ...
  function setMaxMint(uint _maxMint) private onlyOwner() {
    ...
    function build(uint256 tokenAmount, uint256 buildTokenId) external returns (bool) {
		buildTokenContract.safeTransferFrom(msg.sender, address(this), buildTokenId); //transfer token vs burn
        //buildTokenContract.burnBuildToken(buildTokenId, to); //from other contract burn token vs transfer
