...
interface ITombstone {
	...
	function changeActiveTombstone(uint _newTombstone) external;
}

contract NFTombstone is Ownable {
  using Strings for uint256;
  ...
  uint256 public engraveCost = 0.01 ether;
  ...
  IERC721 tombstone = IERC721(nfTombstoneContractAddress);
  ...
  constructor() {  
    ...
    function changeActiveTombstone(uint _newTombstone) public {
        ...
        require(tombstone.ownerOf(_newTombstone) == msg.sender, "That's not your tombstone!");
    ...
    function engraveTombstone(uint tokenID, string memory newTokenUri) public payable {
		ITombstone c = ITombstone(nfTombstoneContractAddress);
        require(msg.sender == tombstone.ownerOf(tokenID), "You don't own this NFT"); 
        require(msg.value >= engraveCost, "Insufficient funds");
        require(hasBeenEngraved[tokenID] == 0, "Tombstone has already been Engraved");
        c.setTokenURI(tokenID, newTokenUri);
        addHasBeenEngraved(tokenID);
        // set tombstone to address when engraved
        addressToTombstone[msg.sender] = tokenID;      
    ...
    function transferTombstoneOwnership(address _contract, address _newOwner) public onlyOwner {
