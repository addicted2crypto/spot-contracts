...
contract AvaxTrucks is ERC721URIStorage, IERC721Receiver, ReentrancyGuard, Ownable, KindaRandom {
  ...
  address public adminWalletAddress = 0xFcf09d15151D4ceBf2844c70bB7a27F1c332f3D3;  
  ...
  constructor(address _freeMintCollectionContract, uint256 _royaltyAmount, address payable _artistWallet, address payable _theSpotWallet, address payable _adminWallet) ERC721("Avax Trucks", "AvaxTrucks") KindaRandom(3500) {
	...
	modifier canMintWithNFT(uint256 nftTokenId) {
		require(_mintedTokensCount[_currentContractAddress][msg.sender] < 2, "You have already minted 2 tokens for this tokenID and contract address.");
		require(
			IERC721(freeMintCollectionContract).ownerOf(nftTokenId) == msg.sender,
			"You do not own this NFT from the collection that is eligible for a free mint."
		);
		_;
	}
	...
	function updateFreeMintCollectionContract(address newContractAddress) external onlyOwner {
        freeMintCollectionContract = newContractAddress;
    }
   ...
   function setMintFee(uint256 mintFee_) public onlyOwner {
	...
	function mintWithNFT(uint256 nftTokenId, address contractAddress) external canMintWithNFT(nftTokenId) {
		...
		if (contractAddress != _currentContractAddress) {
			// Switching to a new contract, update the current contract address and remember the minted tokens from the previous contract
			_currentContractAddress = contractAddress;
			_mintedTokensCount[_currentContractAddress][msg.sender] = _mintedTokensCount[_currentContractAddress][msg.sender];
		}

		// Check if the token ID has already been minted for the specific contract
    ...
    function royaltyInfo(uint256, uint256 value) external view returns(address, uint256) {
        return (treasuryWallet, value * royaltyAmount / 10000);
    } 
